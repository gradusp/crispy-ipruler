...
IPRULER-MAIN:=$(CURDIR)/cmd/ipruler
IPRULER-BIN:=$(CURDIR)/bin/ipruler

.PHONY: build-ipruler
build-ipruler: go-deps
	$(info building 'ipruler' server...)
	@go build -ldflags="$(LDFLAGS)" -o $(IPRULER-BIN) $(IPRULER-MAIN)

.PHONY: build-ipruler-d
build-ipruler-d:
	$(info building 'ipruler-debug' server...)
	@go build -ldflags="$(LDFLAGS)" -gcflags="all=-N -l" -o $(IPRULER-BIN)-dbg $(IPRULER-MAIN)
